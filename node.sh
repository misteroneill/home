...
HAS_LOAD_NVMRC=`cat ${HOME}/.zshrc | grep load-nvmrc`

if [ $HAS_LOAD_NVMRC ]; then
  echo ".zshrc already has load-nvmrc, skipping "
else
  cat ${SRC_DIR}/dotfiles/zshrc-nvm-append.sh >> ${HOME}/.zshrc
fi